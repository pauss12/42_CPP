...
FragTrap::FragTrap(const FragTrap &orig) : ClapTrap(orig)
{
	...
	std::cout << BLUE << "FragTrap " << this->getName() << " has been created with copy constructor" << RESET << std::endl;
}
...
{
	std::cout << LIGHT_BLUE << "FragTrap " << this->getName() << " says HIGH FIVE GUYS!" << RESET << std::endl;
}
